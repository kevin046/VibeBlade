...
void rms_norm(const uint16_t* x, const uint16_t* weight, uint16_t* out,
    ...
    if (!x_f32 || !w_f32 || !o_f32) { aligned_free(x_f32); aligned_free(w_f32); aligned_free(o_f32); throw std::bad_alloc(); }
...
#else
        for (i = 0; i < D; i++) ss += x_f32[i] * x_f32[i];
#endif
...
#else
        for (i = 0; i < D; i++) o_f32[i] = x_f32[i] * inv_rms * w_f32[i];
#endif
